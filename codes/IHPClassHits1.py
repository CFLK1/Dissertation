...
output = "./norange/IHPv2_hadAbortion_hitsAt1_norange.csv"
...
################################################################################
    ...
    hitsAt1 = [set for set in scoreChunk if "score.set.TST.rel.hadAbortion.tail.hits@1" in set]

    # split chunk of text into distinct values
    hitsAt1 = hitsAt1[0].split(" = ")
    hitsAt1 = ["hadAbortionHitsAt1"] + [hitsAt1[1]]
