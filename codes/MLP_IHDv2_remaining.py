...
import ast # for converting class string back to list
...
def calculate_class_weights(y_true):
    number_grp = np.shape(y_true)[1]
    weights = np.empty([number_grp, 2])
    for i in range(number_grp):
...
################################################################################
############################# One-hot prediction for multilabel ############################
################################################################################

def multi_label_hot(prediction, threshold=0.5):
    #prediction = tf.cast(prediction, tf.float32)
    threshold = float(threshold)
    return np.greater(prediction, threshold).astype(int)
...
def MLP(name, input_dir, best_dir, output):
    ...
    for model in models:
        ...
        for i, file in enumerate(files):
            ...
            # convert class string back to a list
            dataset["class"] = dataset["class"].apply(lambda x: ast.literal_eval(x))
...
################################################################################
            ...
            classifier.compile(optimizer=sgd, loss=weighted_BCE(weights),
...
################################################################################
            ...
            custom_objects={"weighted_loss":weighted_BCE(weights)})
            ...
            y_test2 = mlb.transform(y_test2)
            pred_logit = final_model.predict(X_test, verbose=0) # for multi-label
            y_pred = multi_label_hot(pred_logit)
...
MLP(name = "IHDv2", \
input_dir = "./norange/IHDv2_untested (copy)", \
best_dir = "./norange/MLP_models_remaining", \
output = "./norange/IHDv2_remaining_MLPacc_norange_2.csv")
