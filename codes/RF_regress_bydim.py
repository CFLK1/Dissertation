...
def HP_regress(dataframe, target, outdir, dataset, ABType=True):
...
################################################################################
    ...
    X = df.iloc[:,:7].drop("dimension", axis=1)
    y = df[target]
    ...
    if ABType == True:
        ...
        [10,100,1000],[1,2,4],["fixed","max"],["L0","L2","L1"]])
...
################################################################################
    ...
    if ABType == True:
        ...
        xlab = np.where(indices==0, "ont. constr.", indices)
        xlab = np.where(xlab=="1", "learn. rate", xlab)
        xlab = np.where(xlab=="2", "margin", xlab)
        xlab = np.where(xlab=="3", "magnitude", xlab)
        xlab = np.where(xlab=="4", "method", xlab)
...
################################################################################
    ...
    if ABType == True:
        df2.columns = ["HType", "ABType", "learnFac", "margin", "constr", "LType", target]
        mod_feats = ["HType", "ABType", "learnFac", "margin", "constr", "LType"]
    else:
        df2.columns = ["HType", "learnFac", "margin", "constr", "LType", target]
        mod_feats = ["HType", "learnFac", "margin", "constr", "LType"]
    ...
    center=False, plot_lines=False, \
    plot_pts_dist=False, figsize = (3.6,3.6))
    ax3["pdp_ax"].set_xticklabels(["0.1", "0.01", "0.001"])
    ax3["pdp_ax"].set_xticks([0, 1, 2])
    if target is not "mrank":
    ...
    center=False, plot_lines=False, \
    plot_pts_dist=False, figsize = (3.6,3.6))
    ax4["pdp_ax"].set_xticklabels(["1", "2", "4"])
    ...
    center=False, plot_lines=False, \
    plot_pts_dist=False, figsize = (3.6,3.6))
    ax5["pdp_ax"].set_xticklabels(["surface", "space"])
    ax5["pdp_ax"].set_xticks([0, 1])
    if target is not "mrank":
    ...
    center=False, plot_lines=False, \
    plot_pts_dist=False, figsize = (3.6,3.6))
    ax6["pdp_ax"].set_xticklabels(["linear", "projection", "hybrid"])
    ax6["pdp_ax"].set_xticks([0, 1, 2])
    if target is not "mrank":
    ...
    if ABType == True:
        ...
        center=False, plot_lines=False, \
        plot_pts_dist=False, figsize = (3.6,3.6))
        ax7["pdp_ax"].set_xticklabels(["False", "True"])
        ax7["pdp_ax"].set_xticks([0, 1])
        if target is not "mrank":
...
################################################################################
    ...
    fig3_path = "/".join([subdir, "_".join([dataset, target, "PDP_learnFac.png"])])
    fig3.savefig(fig3_path)

    fig4_path = "/".join([subdir, "_".join([dataset, target, "PDP_margin.png"])])
    fig4.savefig(fig4_path)

    fig5_path = "/".join([subdir, "_".join([dataset, target, "PDP_constr.png"])])
    fig5.savefig(fig5_path)

    fig6_path = "/".join([subdir, "_".join([dataset, target, "PDP_LType.png"])])
    fig6.savefig(fig6_path)

    if ABType == True:
        fig7_path = "/".join([subdir, "_".join([dataset, target, "PDP_ABType.png"])])
        fig7.savefig(fig7_path)

################################################################################

# dataframe:    splitted dataframe (jointable format)
...
for index, ds in enumerate(dss):
    ...
    dfpath = "./norange/new_remain/" + ds + "_jointable_norange.csv"
    df = pd.read_csv(dfpath, index_col=0)

    df8 = df[df["dimension"] == 8].reset_index(drop=True)
    df16 = df[df["dimension"] == 16].reset_index(drop=True)
    df32 = df[df["dimension"] == 32].reset_index(drop=True)
    df64 = df[df["dimension"] == 64].reset_index(drop=True)

    dflist = [df8, df16, df32, df64]
    dim = ["8", "16", "32", "64"]
    targets = ["mrr", "mrank", "MLP_acc", "MLP_wF1"]

    for i, d in enumerate(dflist):
        dataset = "_".join([ds, dim[i]])

        for target in targets:
            HP_regress(dataframe=d, #<---------------------------------CHANGE
            target=target, \
            outdir="./norange/new_remain", \
            dataset=dataset, \
            ABType=True if index < 4 else False)
